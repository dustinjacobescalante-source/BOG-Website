...
import MobileHabitDayPicker from '@/components/accountability/MobileHabitDayPicker';
...
function ProgressCard({
  ...
  return (
    <div
      className={`group relative h-full overflow-hidden rounded-[28px] border border-white/10 bg-[#0b1220]/85 p-5 transition-all duration-300 hover:-translate-y-1 hover:border-white/20 ${styles.glow}`}
    >
        ...
        <div className="space-y-3">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              <div className="text-[1.45rem] font-bold leading-tight text-white sm:text-[1.65rem]">
                {title}
              </div>
...
function DashboardNudges({
  ...
  return (
    <Card>
      <div className="space-y-4">
        <div className={`rounded-[22px] border px-4 py-4 ${headlineClass}`}>
          <div className="text-[10px] uppercase tracking-[0.16em] opacity-80">
            Tracker Status
          </div>
          <div className="mt-2 text-lg font-bold">{headline}</div>
...
function HabitSummaryCard({ habits }: { habits: HabitEntry[] }) {
  ...
  return (
    <Card>
      <div className="relative overflow-hidden rounded-[28px] border border-white/10 bg-[#0b1220]/90 p-5 shadow-[0_18px_50px_rgba(0,0,0,0.28)] sm:p-6">
      ...
      <div className="space-y-4">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
          <div>
            ...
            <div className="mt-1 text-sm text-zinc-400">
              Set a monthly target for each habit, mark completed days, and track
              your pace against the goal.
            </div>
          </div>
        ...
        <div className="space-y-4 lg:hidden">
          {rows.map((habit, rowIndex) => {
            ...
            return (
              <div
                key={rowIndex}
                className="overflow-hidden rounded-[26px] border border-white/10 bg-[#0b0f16] p-4 shadow-[0_0_0_1px_rgba(255,255,255,0.02),0_16px_36px_rgba(0,0,0,0.26)]"
              >
                <div className="mb-4 flex items-center justify-between gap-3">
                  ...
                  <div
                    className={`rounded-full border px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] ${rowStyles.labelClass}`}
                  >
                    {rowStyles.label}
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="rounded-2xl border border-white/10 bg-black/20 p-3">
                      ...
                      <div
                        className={`mt-3 inline-flex rounded-2xl border px-3 py-2 text-lg font-bold ${rowStyles.pill}`}
                      >
                        {completedDays}
                      </div>
                    </div>
                  ...
                  <div className="rounded-2xl border border-white/10 bg-black/20 p-4">
                    <div className="mb-2 flex items-center justify-between text-xs text-zinc-400">
                      ...
                      <span>{goalDays > 0 ? `${percentToGoal}%` : '0%'}</span>
                    </div>
                    <div className="h-2.5 rounded-full bg-white/10">
                      <div
                        className={`h-2.5 rounded-full transition-all ${rowStyles.progress}`}
                        style={{ width: `${goalDays > 0 ? percentToGoal : 0}%` }}
                      />
                    </div>
                  </div>

                  <MobileHabitDayPicker
                    rowIndex={rowIndex}
                    initialCheckedDays={checkedDays}
                    disabled={disabled}
                    daysInTracker={daysInTracker}
                  />
                </div>
              </div>
            );
        ...
        <div className="hidden lg:block">
          <div className="rounded-[30px] border border-white/10 bg-[#0b0f16] p-3 shadow-[0_0_0_1px_rgba(255,255,255,0.02),0_18px_40px_rgba(0,0,0,0.30)]">
            <div className="max-h-[70vh] overflow-auto rounded-[22px]">
              <table className="min-w-[1470px] w-full border-separate border-spacing-0 text-left text-sm text-zinc-300">
                <thead>
                  <tr>
                    <th className="sticky left-0 top-0 z-40 min-w-[220px] border-b border-white/10 bg-[#0b0f16] px-3 py-3 text-xs font-semibold uppercase tracking-[0.14em] text-zinc-400">
                      Action / Habit
                    </th>
                    <th className="sticky top-0 z-30 min-w-[110px] border-b border-white/10 bg-[#111827] px-2 py-3 text-center text-[11px] font-semibold uppercase tracking-[0.14em] text-zinc-400">
                      Goal
                    </th>
                    {Array.from({ length: daysInTracker }, (_, dayIndex) => (
                      <th
                        key={dayIndex + 1}
                        className="sticky top-0 z-30 w-[32px] min-w-[32px] border-b border-white/10 bg-[#111827] px-1 py-3 text-center text-[10px] font-semibold text-zinc-400"
                      >
                        {dayIndex + 1}
                      </th>
                    ))}
                    <th className="sticky top-0 z-30 min-w-[72px] border-b border-white/10 bg-[#111827] px-2 py-3 text-center text-[11px] font-semibold uppercase tracking-[0.14em] text-zinc-400">
                      Done
                    </th>
                    <th className="sticky top-0 z-30 min-w-[120px] border-b border-white/10 bg-[#111827] px-2 py-3 text-center text-[11px] font-semibold uppercase tracking-[0.14em] text-zinc-400">
                      Progress
                    </th>
                  </tr>
                </thead>
                  ...
                  {rows.map((habit, rowIndex) => {
                    ...
                    return (
                      <tr key={rowIndex} className={rowStyles.row}>
                        <td className="sticky left-0 z-20 border-b border-white/5 bg-[#0b0f16] px-3 py-2 align-middle">
                          <input
                            type="text"
                            name={`habit_${rowIndex}_action`}
                            defaultValue={habit?.action ?? ''}
                            disabled={disabled}
                            placeholder={`Habit ${rowIndex + 1}`}
                            className="w-full rounded-xl border border-white/10 bg-black/40 px-3 py-2 text-sm text-white outline-none placeholder:text-zinc-500 disabled:opacity-60"
                          />
                        </td>

                        <td className="border-b border-white/5 bg-[#111827] px-2 py-2 text-center align-middle">
                          <input
                            type="number"
                            min={0}
                            name={`habit_${rowIndex}_goal_days`}
                            ...
                            disabled={disabled}
                            placeholder="0"
                            className="w-full rounded-xl border border-white/10 bg-black/40 px-2 py-2 text-center text-sm text-white outline-none placeholder:text-zinc-500 disabled:opacity-60"
                          />
                        </td>
                        ...
                        {Array.from({ length: daysInTracker }, (_, dayIndex) => {
                          ...
                          return (
                            <td
                              key={day}
                              className="border-b border-white/5 bg-[#111827] px-1 py-2 text-center align-middle"
                            >
                              <input
                                type="checkbox"
                        ...
                        <td className="border-b border-white/5 bg-[#111827] px-2 py-2 text-center align-middle">
                          <div
                            className={`rounded-xl border px-2 py-1.5 text-xs font-bold ${rowStyles.pill}`}
                          >
                            {completedDays}
                          </div>
                        </td>
                        ...
                        <td className="border-b border-white/5 bg-[#111827] px-2 py-2 text-center align-middle">
                          <div className="space-y-1.5">
                            ...
                            <div
                              className={`rounded-full border px-2 py-1 text-[9px] font-semibold uppercase tracking-[0.12em] ${rowStyles.labelClass}`}
                            >
                              {rowStyles.label}
                            </div>
                            ...
                            <div className="h-1.5 rounded-full bg-white/10">
                              <div
                                className={`h-1.5 rounded-full transition-all ${rowStyles.progress}`}
                                style={{ width: `${goalDays > 0 ? percentToGoal : 0}%` }}
                              />
                            </div>
                          </div>
...
async function saveAccountability(formData: FormData) {
  ...
  const daysInTracker = getDaysInMonth(entry_month, entry_year);
  ...
  if (existingEntryError) {
    console.error('Failed to load existing accountability entry:', existingEntryError);
    redirect('/portal/accountability?error=entry_lookup_failed');
  }
  ...
  if (existingEntry) {
    const { error: updateError } = await supabase
      ...
      .update({
        ...payload,
        streak_count: existingEntry.streak_count ?? streak,
        best_streak: existingEntry.best_streak ?? bestStreak,
      })
      .eq('id', existingEntry.id);

    if (updateError) {
      console.error('Failed to update accountability entry:', updateError);
      redirect('/portal/accountability?error=entry_update_failed');
    }
  } else {
    ...
    if (insertEntryError) {
      console.error('Failed to insert accountability entry:', insertEntryError);
      redirect('/portal/accountability?error=entry_insert_failed');
    }
  }
  ...
  if (deleteHabitsError) {
    console.error('Failed to delete existing accountability habits:', deleteHabitsError);
    redirect('/portal/accountability?error=habit_delete_failed');
  }
  ...
  if (habitRows.length > 0) {
    const { error: insertHabitsError } = await supabase
      .from('accountability_habits')
      .insert(habitRows);

    if (insertHabitsError) {
      console.error('Failed to insert accountability habits:', insertHabitsError);
      redirect('/portal/accountability?error=habit_insert_failed');
    }
  }
...
export default async function Page({
  ...
  searchParams?: Promise<{ saved?: string; month?: string; year?: string; error?: string }>;
}) {
  const supabase = await createClient();
  const user = await requireUser();
  ...
  const monthLabel = new Date(entry_year, entry_month - 1, 1).toLocaleString(
    'en-US',
    {
      month: 'long',
      year: 'numeric',
  ...
  let displayStreak = entry?.streak_count ?? 0;
  let displayBestStreak = entry?.best_streak ?? 0;
  ...
  if (lastEntry) {
    ...
    if (!isCurrentMonthView || !entry) {
      if (monthsSinceLastEntry > 1) {
        ...
        displayStreak = lastEntry.streak_count ?? 0;
      } else {
        displayStreak = lastEntry.streak_count ?? 0;
      }

      displayBestStreak = lastEntry.best_streak ?? lastEntry.streak_count ?? 0;
    }
  }
  ...
  return (
    ...
    >
      <div className="w-full space-y-5">
            ...
            <div className="space-y-4">
              <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                <div className="max-w-[240px]">
                  ...
                  <div className="mt-1 text-sm text-zinc-400">
                    Switch between current and past tracker entries.
                  </div>
                </div>

                <form method="GET" className="w-full max-w-[470px]">
                  <div className="grid gap-3 sm:grid-cols-[1fr_1fr_auto_auto]">
                    ...
                    <div className="flex items-end">
                      <button
                        type="submit"
                        className="rounded-xl border border-white/10 bg-black/30 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-white/5"
                      >
                        View
                      </button>
                    </div>
        ...
        <form action={saveAccountability} className="space-y-5">
          {formLocked && (
          ...
          <Card>
            <div className="space-y-4">
              ...
              <div className="flex flex-col gap-3 pt-2 sm:flex-row sm:items-center">
                ...
                <a
                  href={`/portal/accountability/print?month=${entry_month}&year=${entry_year}`}
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center justify-center rounded-2xl border border-white/10 bg-white/[0.03] px-5 py-3 text-sm font-semibold text-white/80 transition hover:border-white/20 hover:bg-white/[0.06] hover:text-white"
                >
                  Print Accountability Entry
                </a>
              </div>
            </div>
          </Card>
