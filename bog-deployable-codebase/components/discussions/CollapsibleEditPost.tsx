...
import EditPostForm from '@/components/discussions/EditPostForm';

export default function CollapsibleEditPost({
  postId,
  initialBody,
}: {
  postId: string;
  ...
  return (
    <div className="space-y-3">
      ...
      >
        {open ? 'Close Reply Editor' : 'Edit Reply'}
      </button>
      ...
      {open ? (
        <div className="rounded-[24px] border border-white/10 bg-black/20 p-4 shadow-[0_0_0_1px_rgba(255,255,255,0.02),0_12px_30px_rgba(0,0,0,0.25)]">
          <EditPostForm postId={postId} initialBody={initialBody} />
        </div>
      ) : null}
    </div>
  );
